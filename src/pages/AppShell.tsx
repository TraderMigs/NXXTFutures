...
import { Flame, BarChart2, LogOut, History, BookOpen, Menu, X, ChevronRight, Shield, GraduationCap } from 'lucide-react';
...
type Tab = 'hot-picks' | 'data-analysis' | 'journal' | 'history' | 'education' | 'admin';
...
const ADMIN_TAB = { id: 'admin' as Tab, label: 'Admin', icon: <Shield className="w-5 h-5" />, description: 'Platform management' };
              ...
              {TABS.map(tab => (
                <button key={tab.id} onClick={() => handleTabSelect(tab.id)}
                  className={`relative flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                    activeTab === tab.id
                      ? tab.id === 'admin' ? 'bg-purple-500/10 text-purple-400' : 'bg-amber-500/10 text-amber-400'
          ...
          {TABS.map(tab => (
            <button key={tab.id} onClick={() => handleTabSelect(tab.id)}
              ...
              <div className={`flex-shrink-0 ${
                activeTab === tab.id
                  ? tab.id === 'admin' ? 'text-purple-400' : 'text-amber-400'
                  : 'text-gray-600'
              }`}>{tab.icon}</div>
              ...
              <ChevronRight className={`w-4 h-4 flex-shrink-0 ${activeTab === tab.id ? (tab.id === 'admin' ? 'text-purple-400' : 'text-amber-400') : 'text-gray-700'}`} />
            </button>
          ))}
        </nav>
