    ...
    <div className={`relative bg-[#111318] border rounded-2xl overflow-hidden transition-all duration-300 signal-enter ${signal.status === 'ACTIVE' ? `${statusCfg.border} signal-active` : 'border-[#1E2128]'}`}>
      ...
      <div className="p-5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {signal.is_counter_trend && (
                <span className="text-[9px] bg-orange-500/15 text-orange-400 border border-orange-500/20 px-1.5 py-0.5 rounded font-bold">CT ⚠️</span>
              )}
            </div>
            <div className="font-data text-xs text-gray-600">{config?.fullName}</div>
          </div>
          <div className="flex flex-col items-end gap-1.5">
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border ${isBuy ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' : 'bg-red-500/10 text-red-400 border-red-500/30'}`}>
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5">
              <div className="w-20 h-1.5 bg-[#1E2128] rounded-full overflow-hidden">
                <div className={`h-full rounded-full gauge-fill ${signal.confidence >= 85 ? 'bg-emerald-400' : signal.confidence >= 75 ? 'bg-amber-400' : 'bg-orange-400'}`}
          ...
          <div className={`px-2 py-0.5 rounded border text-[10px] font-data font-medium ${sessionColors[signal.session] || sessionColors.OFF}`}>{signal.session}</div>
          {signal.atr_value && (
          ...
          {signal.news_warning && (
            <div className="flex items-center gap-1 px-2 py-0.5 bg-orange-500/10 border border-orange-500/20 rounded text-[10px] text-orange-400">
              <AlertTriangle className="w-3 h-3" /> NEWS
            </div>
          )}
        ...
        <button onClick={() => setShowDetails(!showDetails)}
          ...
          <span>Trade Levels & Details</span>
