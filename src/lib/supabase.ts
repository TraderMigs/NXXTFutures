...
ENTRY TIMEFRAME RULE:
Set "entry_timeframe" to the timeframe the trader should actually be watching to pull the trigger.
- Use "15m" when entry precision is on the 15M chart (most setups)
- Use "1h" when the entry candle itself is on the 1H chart (wider OBs, cleaner 1H setups)
- Use "5m" only for very tight scalp entries with 15M structure
...
async function saveSignal(signal: any, supabase: any): Promise<void> {
  ...
  await supabase.from('futures_signals').insert({
    ...
    entry_timeframe: signal.entry_timeframe || '15m',
